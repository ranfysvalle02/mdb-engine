"""Watch for changes to OSI model files and trigger reloads.
...
class OsiModelWatcher:
    ...
    When a change is detected, reloads the registry.
    """
        ...
        while self._running:
            ...
            try:
                if self.check_for_updates():
                    logger.info("OSI model files changed, reloading...")
                    self._registry.reload()
                    logger.info("OSI models reloaded successfully")
            except (OSError, ValueError, TypeError, RuntimeError) as e:
                logger.warning(f"OSI watcher reload failed: {e}")
