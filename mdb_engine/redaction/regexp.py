"""
Regexp Redaction Service Implementation

Provides regex-based redaction using compiled patterns for common PII types.
This is the default redaction provider and maintains backward compatibility
with the original RedactionService implementation.
"""
...
from .base import BaseRedactionService

logger = logging.getLogger(__name__)


class RegexpRedactionService(BaseRedactionService):
    """
    Regex-based redaction service for protecting sensitive data.

    This service processes text using compiled regex patterns to identify
    and redact common PII types like SSN, credit cards, phone numbers, etc.
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        """
        Initialize RegexpRedactionService with configuration.
        ...
        if self.enabled:
            logger.info(
                f"✅ RegexpRedactionService initialized with {len(self._compiled_patterns)} "
                f"active patterns, replacement='{self.replacement}'"
            )
        else:
            logger.info("⏸️ RegexpRedactionService initialized but DISABLED")
    ...
    def redact(self, text: str) -> str:
        ...
        if self.log_redactions and total_redactions > 0:
            logger.info(f"🔒 Redacted {total_redactions} sensitive items: {redaction_counts}")
