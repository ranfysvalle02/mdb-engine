...
from casbin_motor_adapter import Adapter as CasbinMotorAdapter  # type: ignore
...
async def create_casbin_enforcer(
    ...
    try:
        try:
            adapter = CasbinMotorAdapter(mongo_uri, db_name, policies_collection)
            logger.debug(f"Casbin MotorAdapter created with collection '{policies_collection}'")
        except TypeError:
            logger.warning("Adapter doesn't support custom collection name, using default")
                ...
                if user and role:
                    try:
                        ...
                        if not exists:
                            ...
                            logger.debug(f"  Assigned role '{role}' to '{user}'")
