...
def token_security(enforce_https: bool = True, check_csrf: bool = True):
    ...
    def decorator(func: Callable[..., Awaitable[Any]]) -> Callable[..., Awaitable[Any]]:
        ...
        async def wrapper(request: Request, *args, **kwargs):
            ...
            if check_csrf and request.method in ["POST", "PUT", "DELETE", "PATCH"]:
        ...
        async def wrapper(request: Request, *args, **kwargs):
            ...
            if identifier in _rate_limit_storage:
                ...
                _rate_limit_storage[identifier] = {
                    ts: count for ts, count in attempts.items()
            ...
            if attempts_in_window >= max_attempts_val:
                logger.warning(f"Rate limit exceeded for {identifier}: {attempts_in_window} attempts in {window_seconds_val}s")
                raise HTTPException(
