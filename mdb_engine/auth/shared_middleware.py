...
def _match_route_patterns(path: str, patterns: list[str]) -> bool:
    """Check if *path* matches any of the given route patterns.

    Supported pattern syntax:
        ``/exact``          – exact match
        ``/prefix/*``       – single-segment wildcard (fnmatch ``*``)
        ``/prefix/**``      – multi-segment wildcard (any depth below prefix)
        ``/api/*/items``    – wildcard in the middle
        Standard ``fnmatch`` globs (``?``, ``[seq]``) also work.
    """
    for pattern in patterns:
        ...
        # ** means "match everything below this prefix"
        if "**" in pattern:
...
class SharedAuthMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        Args:
            ...
            public_routes: Route patterns that don't require auth.
                          Supports ``*`` (single segment), ``**`` (any depth),
                          e.g. ``["/health", "/api/**"]``
            protected_routes: Route patterns that *require* auth when
                             ``default_policy`` is ``"public"``.  Mutually
                             exclusive with ``public_routes``.
            default_policy: ``"protected"`` (default) — all routes require auth
                           unless listed in ``public_routes``.
                           ``"public"`` — all routes are public unless listed in
                           ``protected_routes``.
            role_hierarchy: Optional role hierarchy for inheritance
            session_binding: Session binding configuration
            auto_assign_default_role: If True, auto-assign require_role to new users
        ...
        self._protected_routes = protected_routes or []
        ...
        logger.info(
            ...
            f"(require_role={require_role}, default_policy={default_policy}, "
            f"public_routes={len(self._public_routes)}, "
            f"protected_routes={len(self._protected_routes)}, "
    ...
    def _is_public_route(self, path: str) -> bool:
        """Check if path is public based on default_policy and route lists."""
        if self._default_policy == "public":
            return not _match_route_patterns(path, self._protected_routes)
        return _match_route_patterns(path, self._public_routes)
...
def create_shared_auth_middleware(
    ...
    protected_routes = manifest_auth.get("protected_routes", [])
    default_policy = manifest_auth.get("default_policy", "protected")
...
def _is_public_route_helper(path: str, public_routes: list[str]) -> bool:
    ...
    return _match_route_patterns(path, public_routes)
...
def _create_lazy_middleware_class(  # noqa: C901
    ...
    _protected_routes = protected_routes or []
    ...
    class LazySharedAuthMiddleware(BaseHTTPMiddleware):
        ...
        def __init__(self, app: Callable):
            ...
            self._protected_routes = _protected_routes
            self._default_policy = _default_policy
            self._role_hierarchy = role_hierarchy
            ...
            logger.info(
                ...
                f"(require_role={require_role}, default_policy={_default_policy}, "
                f"public_routes={len(self._public_routes)}, "
                f"protected_routes={len(self._protected_routes)}, "
                f"session_binding={bool(self._session_binding)})"
            )
        ...
        async def dispatch(
            ...
            req_path = _get_request_path(request)
            if self._default_policy == "public":
                is_public = not _match_route_patterns(req_path, self._protected_routes)
            else:
                is_public = _match_route_patterns(req_path, self._public_routes)
