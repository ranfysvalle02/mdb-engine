...
def _get_secret_key() -> str:
    ...
    if not secret_key:
        raise ConfigurationError(
            "JWT secret is required. Set MDB_JWT_SECRET (preferred) or "
            "SECRET_KEY with a strong, cryptographically random value "
            "(minimum 32 characters). "
            "Example: export MDB_JWT_SECRET=$(python -c "
            "'import secrets; print(secrets.token_urlsafe(32))')",
            config_key="MDB_JWT_SECRET",
        )
