...
It integrates seamlessly with mdb-runtime's MongoDB connection.
...
class Mem0MemoryService:
    ...
    - Stores and retrieves memories in MongoDB (using mdb-runtime's connection)
    ...
    def __init__(
        ...
        Args:
            ...
            config: Optional memory configuration dict (from manifest.json memory_config)
                   Can include: collection_name, enable_graph, infer, embedding_model, chat_model, temperature, etc.
                   Note: embedding_model_dims is auto-detected by embedding a test string - no need to specify!
                   Embeddings and LLM are configured via environment variables (.env).
        ...
        config_embedding_dims = (config or {}).get("embedding_model_dims")  # Optional - will be auto-detected
        ...
        embedding_model = (config or {}).get("embedding_model") or os.getenv("EMBEDDING_MODEL", "text-embedding-3-small")
        chat_model = (config or {}).get("chat_model") or os.getenv("CHAT_MODEL") or os.getenv("AZURE_OPENAI_DEPLOYMENT_NAME", "gpt-4o")
        ...
        if provider == "azure":
            if not os.getenv("AZURE_OPENAI_API_KEY") or not os.getenv("AZURE_OPENAI_ENDPOINT"):
                raise Mem0MemoryServiceError(
                    f"Azure OpenAI provider requires AZURE_OPENAI_API_KEY and AZURE_OPENAI_ENDPOINT "
                    f"environment variables to be set."
                )
        else:
            if not os.getenv("OPENAI_API_KEY"):
                raise Mem0MemoryServiceError(
                    f"OpenAI provider requires OPENAI_API_KEY environment variable to be set."
                )
        ...
        try:
            ...
            self.embedding_model_dims = detected_dims if detected_dims is not None else (config_embedding_dims or 1536)
            
            mem0_config["vector_store"]["config"]["embedding_model_dims"] = self.embedding_model_dims
            
            # Configure mem0 embedder
            # For Azure, must use azure_openai provider with azure_kwargs
            clean_embedding_model = embedding_model.replace("azure/", "").replace("openai/", "")
            if provider == "azure":
                # Azure OpenAI requires azure_openai provider with azure_kwargs
                azure_endpoint = os.getenv("AZURE_OPENAI_ENDPOINT")
                azure_api_key = os.getenv("AZURE_OPENAI_API_KEY")
                azure_api_version = os.getenv("AZURE_OPENAI_API_VERSION", os.getenv("OPENAI_API_VERSION", "2024-02-15-preview"))
                
                if not azure_endpoint or not azure_api_key:
                    raise Mem0MemoryServiceError(
                        f"Azure OpenAI requires AZURE_OPENAI_ENDPOINT and AZURE_OPENAI_API_KEY environment variables"
                    )
            ...
            logger.info(
                f"Configuring mem0 embedder ({'Azure OpenAI' if provider == 'azure' else 'OpenAI'}): "
                f"provider='{mem0_config['embedder']['provider']}', model='{clean_embedding_model}'",
                extra={
                    "app_slug": app_slug,
            ...
            if self.infer:
                # Clean model name (remove azure/ or openai/ prefix)
                clean_chat_model = chat_model.replace("azure/", "").replace("openai/", "")
                # For Azure, prefer deployment name from env
                if provider == "azure":
                    ...
                    if not azure_endpoint or not azure_api_key:
                        raise Mem0MemoryServiceError(
                            f"Azure OpenAI LLM requires AZURE_OPENAI_ENDPOINT and AZURE_OPENAI_API_KEY environment variables"
                        )
                ...
                logger.info(
                    f"Configuring mem0 LLM ({'Azure OpenAI' if provider == 'azure' else 'OpenAI'}): "
                    f"provider='{mem0_config['llm']['provider']}', model='{clean_chat_model}'",
                    extra={
                        "app_slug": app_slug,
        ...
        if self.enable_graph:
            ...
            if graph_config:
                ...
                logger.warning(
                    f"Graph memory enabled but no graph_store config provided. "
                    f"Graph features will not work. Configure graph_store in manifest.json",
        ...
        try:
            ...
            logger.debug(
                f"Initializing Mem0 Memory with config structure",
                extra={
                    "app_slug": app_slug,
                    ...
                    "llm_provider": mem0_config.get("llm", {}).get("provider") if self.infer else None,
                    "full_config": mem0_config
                }
            )
            
            # Initialize Mem0 Memory instance
            # Use Memory.from_config() which properly handles dict config
            init_method = None
            try:
                # Memory.from_config() is the correct way to initialize with a dict config
                # It internally converts the dict to a Config object
                if hasattr(Memory, 'from_config'):
                    self.memory = Memory.from_config(mem0_config)
                    init_method = "Memory.from_config()"
                else:
                    # Fallback: try to create Config object if from_config doesn't exist
                    # This should not happen with recent mem0 versions
                    try:
                        from mem0.config import Config
                        config_obj = Config(**mem0_config)
                        self.memory = Memory(config_obj)
                        init_method = "Memory(Config())"
                    except (ImportError, TypeError) as config_error:
                        # Last resort: try dict directly (may fail)
                        logger.warning(
                ...
                logger.error(
                    ...
                    extra={
                        ...
                        "config_keys": list(mem0_config.keys()) if isinstance(mem0_config, dict) else "not_dict"
                    }
                )
                raise Mem0MemoryServiceError(
                    f"Failed to initialize Memory instance: {error_msg}. "
                    f"Ensure mem0ai is installed and Azure OpenAI environment variables are set correctly."
                ) from init_error
            ...
            logger.info(
                ...
                extra={
                    ...
                    "embedder_provider": mem0_config.get("embedder", {}).get("provider"),
                    "embedder_model": mem0_config.get("embedder", {}).get("config", {}).get("model"),
                    "llm_provider": mem0_config.get("llm", {}).get("provider") if self.infer else None,
                    "llm_model": mem0_config.get("llm", {}).get("config", {}).get("model") if self.infer else None
                }
            )
        except Exception as e:
            logger.error(
        ...
        **kwargs
        ...
        Args:
            ...
            **kwargs: Additional mem0.add() parameters:
                    ...
                    Note: async_mode is not a valid parameter for Mem0's add() method.
                          Mem0 processes memories asynchronously by default.
                          Graph features are configured at initialization via enable_graph in config,
                          not per-add call.
        ...
        try:
            ...
            # Remove async_mode if it was passed (Mem0 doesn't support it)
            add_kwargs.pop("async_mode", None)
            
            # Note: enable_graph is configured at initialization, not per-add call
            # Graph features are set up during Mem0MemoryService initialization if enable_graph=True in config
            
            # Call mem0.add()
            # Note: Mem0 processes asynchronously by default, so we don't need async_mode parameter
            ...
            logger.info(
                f"🔵 CALLING mem0.add() - app_slug='{self.app_slug}', user_id='{user_id}', messages={len(messages)}, infer={add_kwargs.get('infer', 'N/A')}",
                extra={
            ...
            logger.debug(
                f"Raw result from mem0.add(): type={type(result)}, length={len(result) if isinstance(result, list) else 0}",
                extra={
                    "app_slug": self.app_slug,
                    "user_id": user_id,
            ...
            logger.info(
                ...
                extra={
                    ...
                    "memory_ids": [m.get("id") or m.get("_id") for m in result if isinstance(m, dict)] if result else [],
                    "infer_enabled": add_kwargs.get("infer", False),
                    "has_llm": hasattr(self.memory, 'llm') and self.memory.llm is not None if hasattr(self.memory, 'llm') else False
                }
            )
            ...
            if len(result) == 0 and add_kwargs.get("infer", False):
                ...
                logger.info(
                    f"ℹ️ mem0.add() returned 0 memories. This is normal if the conversation doesn't contain extractable facts. "
                    f"mem0 extracts personal preferences, facts, and details - not generic greetings or small talk. "
                    f"Try conversations like 'I love pizza' or 'I work as a software engineer' to see memories extracted.",
                    extra={
                        "app_slug": self.app_slug,
                        "user_id": user_id,
                        "message_count": len(messages),
    ...
    def get_all(
        ...
        **kwargs
        ...
        try:
            ...
            logger.info(
                f"🟢 CALLING mem0.get_all() - app_slug='{self.app_slug}', user_id='{user_id}' (type: {type(user_id).__name__}), collection='{self.collection_name}'",
                extra={
                    "app_slug": self.app_slug,
                    "user_id": user_id,
            ...
            while attempt <= max_retries:
                if attempt > 0:
                    ...
                    logger.debug(
                        ...
                        extra={"app_slug": self.app_slug, "user_id": user_id, "attempt": attempt + 1}
                    )
                
                # Call with safety - catch any exceptions from mem0
                try:
                    logger.debug(
                        f"🟢 EXECUTING: memory.get_all(user_id='{user_id}', limit={limit}, kwargs={kwargs})",
                        extra={
                            "app_slug": self.app_slug,
                            "user_id": user_id,
                            "collection_name": self.collection_name,
                    ...
                    result = self.memory.get_all(user_id=str(user_id), limit=limit, **kwargs)  # Ensure string
                    logger.debug(
                        f"🟢 RESULT RECEIVED: type={type(result).__name__}, length={len(result) if isinstance(result, (list, dict)) else 'N/A'}",
                        extra={
                            "app_slug": self.app_slug,
                            "user_id": user_id,
                    ...
                    logger.error(
                        ...
                        extra={"app_slug": self.app_slug, "user_id": user_id, "error": str(attr_error), "attempt": attempt + 1}
                    )
                    ...
                    logger.error(
                        ...
                        exc_info=True,
                        extra={"app_slug": self.app_slug, "user_id": user_id, "error": str(mem0_error), "attempt": attempt + 1}
                    )
                ...
                if isinstance(result, dict):
                    if "results" in result:
                        ...
                        logger.debug(
                            f"Extracted results from dict response",
                            extra={
                                "app_slug": self.app_slug,
                                "user_id": user_id,
            ...
            logger.info(
                f"Retrieved {len(result)} memories for user '{user_id}' (after {attempt + 1} attempt(s))",
                extra={
                    "app_slug": self.app_slug,
                    "user_id": user_id,
                    "memory_count": len(result) if isinstance(result, list) else 0,
                    "attempts": attempt + 1,
                    "sample_memory": result[0] if result and isinstance(result, list) and len(result) > 0 else None
                }
            )
    ...
    def search(
        ...
        **kwargs
        ...
        Args:
            ...
            metadata: Optional metadata dict to filter results (e.g., {"category": "travel"})
                     Deprecated in favor of 'filters' parameter for Mem0 1.0.0+
            filters: Optional enhanced filters dict (Mem0 1.0.0+) with operators like {"category": {"eq": "travel"}}
            ...
            if user_id and isinstance(result, dict):
                ...
                if result_user_id and result_user_id != user_id:
                    logger.warning(
                        ...
                        extra={"memory_id": memory_id, "user_id": user_id, "result_user_id": result_user_id}
                    )
                    raise Mem0MemoryServiceError(f"Memory {memory_id} does not belong to user {user_id}")
