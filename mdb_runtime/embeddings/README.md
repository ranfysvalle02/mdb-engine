...
Semantic text splitting and embedding generation for MDB_RUNTIME applications.
...
```python
from mdb_runtime.embeddings import EmbeddingService, OpenAIEmbeddingProvider, EmbeddingProvider
...
from mdb_runtime.embeddings.dependencies import get_embedding_service_dependency
from mdb_runtime.embeddings import EmbeddingService
...
```python
from mdb_runtime.embeddings import EmbeddingServiceError
...
- For memory functionality, use `mdb_runtime.memory.Mem0MemoryService` which handles embeddings and LLM via environment variables
