...
from ..constants import MAX_SESSIONS_PER_USER, SESSION_INACTIVITY_TIMEOUT
from ..config import MAX_SESSIONS_PER_USER as CONFIG_MAX_SESSIONS, SESSION_INACTIVITY_TIMEOUT as CONFIG_INACTIVITY_TIMEOUT
...
class SessionManager:
    ...
    async def get_session_by_refresh_token(self, refresh_jti: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error getting session for refresh token {refresh_jti}: {e}", exc_info=True)
            return None
    ...
    async def validate_session_fingerprint(
        ...
        Args:
            ...
            strict: If True, reject if fingerprint doesn't match. If False, allow if no fingerprint stored.
    ...
    async def cleanup_inactive_sessions(self, user_id: Optional[str] = None) -> int:
        ...
        try:
            ...
            result = await self.collection.update_many(
                query,
                {"$set": {"active": False, "revoked_at": datetime.utcnow(), "reason": "inactivity"}}
            )
    ...
    async def revoke_session_by_refresh_token(self, refresh_jti: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error revoking session by refresh token {refresh_jti}: {e}", exc_info=True)
            return False
