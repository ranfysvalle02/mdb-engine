...
Comprehensive authentication and authorization system for MDB_RUNTIME applications with JWT tokens, pluggable authorization providers, session management, and FastAPI integration.
...
The auth module is part of MDB_RUNTIME. No additional installation required, but you may need:
...
Configure authentication in your `manifest.json`:
...
# Setup auth from manifest (in startup)
...
```python
from mdb_runtime.auth import generate_token_pair, encode_jwt_token
...
```python
from mdb_runtime.auth import decode_jwt_token, extract_token_metadata
...
```python
from mdb_runtime.auth import require_auth
...
```python
from mdb_runtime.auth import require_admin
...
```python
from mdb_runtime.auth import require_permission, get_authz_provider
from fastapi import Depends
...
enforcer = casbin.AsyncEnforcer("model.conf", "policy.csv")
...
```python
from mdb_runtime.auth import OsoAdapter
from oso import Oso
...
```python
from mdb_runtime.auth import SessionManager
...
```python
from mdb_runtime.auth import TokenBlacklist
...
```python
from mdb_runtime.auth import create_security_middleware, add_security_middleware
...
from mdb_runtime.auth import SecurityMiddleware
...
```python
from mdb_runtime.auth import token_security
...
```python
from mdb_runtime.auth import rate_limit_auth
...
from mdb_runtime.auth import setup_auth_from_manifest
