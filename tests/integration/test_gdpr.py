...
@pytest.mark.integration
@pytest.mark.asyncio
class TestGDPRDeletionCompleteness:
    """Verify GDPR deletion removes ALL user data across ALL collections."""

    async def test_hard_delete_removes_all_user_data_across_collections(self, real_mongodb_engine):
        """Hard delete must remove target user data from every collection while preserving others."""
        engine = real_mongodb_engine
        db = await engine.get_scoped_db("test_app")
        ...
        # Seed target user data into three separate collections
        await db.user_profiles.insert_one(
            {"user_id": target_user_id, "name": "Target User", "email": "target@example.com"}
        )
        await db.chat_history.insert_one(
            {"user_id": target_user_id, "message": "Hello from target", "ts": "2025-01-01"}
        )
        await db.activities.insert_one({"user_id": target_user_id, "action": "login", "ip": "10.0.0.1"})

        # Seed a different user's data (must survive the deletion)
        await db.user_profiles.insert_one(
            {"user_id": other_user_id, "name": "Other User", "email": "other@example.com"}
        )
        await db.chat_history.insert_one({"user_id": other_user_id, "message": "Hello from other", "ts": "2025-01-02"})
        await db.activities.insert_one({"user_id": other_user_id, "action": "signup", "ip": "10.0.0.2"})

        # Execute hard delete for the target user
        result = await engine.delete_user_data(
        ...
        assert result["documents_deleted"] > 0
        assert len(result["collections_processed"]) > 0

        # Target user's data must be gone from ALL collections
        assert await db.user_profiles.count_documents({"user_id": target_user_id}) == 0
        assert await db.chat_history.count_documents({"user_id": target_user_id}) == 0
        assert await db.activities.count_documents({"user_id": target_user_id}) == 0

        # Other user's data must be intact
        assert await db.user_profiles.count_documents({"user_id": other_user_id}) == 1
        assert await db.chat_history.count_documents({"user_id": other_user_id}) == 1
        assert await db.activities.count_documents({"user_id": other_user_id}) == 1

        # Cleanup remaining data
        await db.user_profiles.delete_many({"user_id": other_user_id})
        await db.chat_history.delete_many({"user_id": other_user_id})
        await db.activities.delete_many({"user_id": other_user_id})
