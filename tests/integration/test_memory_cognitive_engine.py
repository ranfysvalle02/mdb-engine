...
class TestCognitiveEngineIntegration:
        ...
        try:
            ...
            with (
                ...
                async with app.router.lifespan_context(app):
                    ...
                    motor_client = engine.connection_manager.mongo_client
                    ...
                    mock_llm_provider.generate_chat_completion = MagicMock(
                    ...
                    with patch(
                        "mdb_engine.memory.cognitive.completion",
                        ...
                        assert (
                            result["response"] == "I understand you love chocolate. That's great!"
                        )
                        ...
                        assert any(
                            "chocolate" in m.get("memory", "").lower() for m in stored_memories
                        )

                        # Test retrieval in next chat
                        mock_completion_response.choices[0].message.content = '{"facts": []}'
                        ...
                        if len(result2["ltm_memories"]) == 0:
                            ...
                            assert (
                                len(all_memories) > 0
                            ), "Memories should exist in database even if search failed"
                            ...
                            assert (
                                len(result2["ltm_memories"]) > 0
                            ), "Should retrieve stored memories"
    ...
    @pytest.mark.asyncio
    async def test_cognitive_engine_stm_context_management(
        self, mongodb_connection_string, temp_manifest_with_memory
    ):
        ...
        try:
            ...
            with (
                ...
                assert mock_llm_provider.generate_chat_completion.called
                call_args = mock_llm_provider.generate_chat_completion.call_args
...
class TestMultiAppMemoryIsolation:
    ...
    @pytest.mark.asyncio
    async def test_memory_isolation_between_apps(
        self, mongodb_connection_string, temp_manifests_multi_app
    ):
        ...
        try:
            ...
            with (
                ...
                async with app.router.lifespan_context(app):
                    ...
                    # Get collections for CognitiveEngine
                    motor_client = engine.connection_manager.mongo_client
                    pymongo_client = motor_client.delegate
                    pymongo_db = pymongo_client[engine.db_name]

                    chat_history_app1 = pymongo_db["chat_history"]
                    chat_history_app2 = pymongo_db["chat_history"]

                    # Mock LLM providers
                    mock_llm_provider = MagicMock()
                    mock_llm_provider.generate_chat_completion = MagicMock(return_value="Response")
                    ...
                    async def mock_chat_completion_app1(*args, **kwargs):
                        return (
                            '{"facts": [{"text": "User loves app1", '
                            '"category": "preferences", "emotion": 0.5}]}'
                        )

                    async def mock_chat_completion_app2(*args, **kwargs):
                        return (
                            '{"facts": [{"text": "User loves app2", '
                            '"category": "preferences", "emotion": 0.5}]}'
                        )

                    mock_llm_service_app1.chat_completion = MagicMock(
                        side_effect=mock_chat_completion_app1
                    )
                    mock_llm_service_app2.chat_completion = MagicMock(
                        side_effect=mock_chat_completion_app2
                    )
                    ...
                    with patch(
                        ...
                        memory_service_app1._injected_llm_service.chat_completion = MagicMock(
                            side_effect=mock_chat_app1
                        )
                        memory_service_app2._injected_llm_service.chat_completion = MagicMock(
                            side_effect=mock_chat_app2
                        )
                        ...
                        mock_completion_response.choices[0].message.content = fact_json_app1
                        ...
                        mock_completion_response.choices[0].message.content = fact_json_app2
                        ...
                        assert all(
                            "app2" in str(m.get("metadata", {}))
                            or "app1" not in str(m.get("memory", ""))
        ...
        try:
            ...
            with (
                ...
                async with app.router.lifespan_context(app):
                    ...
                    ), "Memory service should be initialized with SSO"
                    ...
                    mock_llm_provider = MagicMock()
                    mock_llm_provider.generate_chat_completion = MagicMock(
                        return_value="SSO memory test response"
                    )
                ...
                async with app.router.lifespan_context(app):
                    ...
                    if hasattr(memory_service, "persona_engine") and memory_service.persona_engine:
                        memory_service.persona_engine.update_persona(
                            ...
                            description=(
                                "Expert in Python and system design. " "Concise and technical."
                            ),
                    ...
                    with patch(
                        ...
                        # Note: add() is synchronous, but we're in async context
                        # so we use add_async() instead
                        ...
                        # Verify system prompt was enhanced (check LLM provider call)
                        assert mock_llm_provider.generate_chat_completion.called
                        call_args = mock_llm_provider.generate_chat_completion.call_args
                        ...
                        # Verify context-engineered prompt structure
                        if result["persona_used"]:
                            assert (
                                "[PERSONA LAYER]" in system_prompt
                                or "Senior Python Architect" in system_prompt
                            )

                        if result["entity_facts"]:
                            assert (
                                "[USER CONTEXT]" in system_prompt or "Known Facts" in system_prompt
                            )

                        if result["dynamic_instructions"]:
                            assert (
                                "[META-INSTRUCTIONS]" in system_prompt
                                or "expert" in system_prompt.lower()
                            )
