...
class TestWebSocketWithCORSCConfig:
    ...
    @pytest.mark.asyncio
    async def test_websocket_connection_with_cors_config(self, mongodb_connection_string, tmp_path, monkeypatch):
        """Test that WebSocket connections work with proper CORS config propagation."""
        import os

        from mdb_engine.core.engine import MongoDBEngine

        # Suppress auto-added dev origins so we test pure CORS merge logic
    ...
    @pytest.mark.asyncio
    async def test_cors_wildcard_merging(self, mongodb_connection_string, tmp_path, monkeypatch):
...
@pytest.mark.integration
class TestCORSWithRealHTTPRequests:
    """Verify CORS headers in actual HTTP responses."""

    @pytest.mark.asyncio
    async def test_cors_allowed_and_disallowed_origins(self, mongodb_connection_string, tmp_path, monkeypatch):
        """Allowed origins get CORS headers; disallowed origins do not."""
        from httpx import ASGITransport, AsyncClient
        ...
        db_name = f"test_cors_http_{os.getpid()}"
        engine = MongoDBEngine(mongo_uri=mongodb_connection_string, db_name=db_name)
            ...
            apps=[
                {
                    "slug": "cors-http-app",
                    "manifest": manifest_path,
                    "path_prefix": "/cors-app",
                }
            ],
            title="CORS HTTP Test",
        )
        ...
        async with app.router.lifespan_context(app):
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
                ...
                ), f"Expected allowed origin header, got: {dict(response.headers)}"
                assert (
                ...
                assert acao != "https://evil.com", (
                    f"Disallowed origin must NOT receive matching CORS header, "
                    f"but got access-control-allow-origin={acao}"
                )
