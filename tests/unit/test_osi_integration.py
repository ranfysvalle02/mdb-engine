...
def _make_registry_with_models(models: list[dict[str, Any]], app_slug: str = "test") -> Any:
    """Create an OsiModelRegistry pre-loaded with models (no loader/YAML needed)."""
    from mdb_engine.osi.registry import OsiModelRegistry

    reg = OsiModelRegistry(app_slug=app_slug, config={"enabled": True})
    reg._models = list(models)
    reg._index_models()
...
class TestOsiModelRegistry:
    ...
    @pytest.fixture
    def registry(self):
        """Create a registry with pre-loaded models."""
        return _make_registry_with_models([_sample_osi_model()], app_slug="test_app")

    def test_load_indexes_models(self, registry):
    ...
    def test_get_node_types(self, registry):
        """Node types extracted from datasets."""
        types = registry.get_node_types()
        assert "customer" in types
        assert "order" in types

    def test_get_all_metric_keywords(self, registry):
    ...
    def test_list_datasets(self, registry):
    ...
    def test_reload(self, registry):
        """Pre-loaded registry is indexed and ready."""
        assert registry.loaded
        assert len(registry.models) == 1
...
class TestEntityResolver:
    ...
    @pytest.fixture
    def registry(self):
        return _make_registry_with_models([_sample_osi_model()])
...
class TestSchemaGrounder:
    ...
    async def test_ground_with_osi_registry(self):
        ...
        registry = _make_registry_with_models([_sample_osi_model()])
...
class TestOsiManifestSchema:
    ...
    @pytest.mark.asyncio
    async def test_osi_config_rejects_inline_semantic_models(self):
        """osi_config rejects inline semantic_models (YAML-only)."""
        from mdb_engine.core.manifest import ManifestValidator
        ...
        validator = ManifestValidator()
        is_valid, error, paths = await validator.validate(manifest)
        assert not is_valid, "semantic_models should be rejected (YAML-only)"
...
class TestExpandedLoader:
    """Tests for the YAML-only OSI loader."""
    ...
    def test_inline_semantic_models_ignored(self):
        """Inline semantic_models in config are no longer loaded (YAML-only)."""
        from mdb_engine.osi.loader import load_all_models
...
# ===========================================================================
# Loader DX: Path validation & file provenance tests
# ===========================================================================


class TestLoaderPathValidation:
    """Tests for loader path existence warnings and zero-model catch-all."""

    def test_nonexistent_models_path_warns(self, tmp_path, caplog):
        """Nonexistent models_path logs a clear warning."""
        import logging

        from mdb_engine.osi.loader import load_all_models

        bogus_path = str(tmp_path / "sematic_models")  # typo
        config = {"enabled": True, "models_path": bogus_path}
        ...
        assert bogus_path in caplog.text

    def test_empty_directory_warns(self, tmp_path, caplog):
        """models_path with no YAML files logs a warning."""
        import logging

        from mdb_engine.osi.loader import load_all_models

        empty_dir = tmp_path / "semantic_models"
        empty_dir.mkdir()
        config = {"enabled": True, "models_path": str(empty_dir)}

        with caplog.at_level(logging.WARNING):
            models = load_all_models(config)

        assert models == []
        assert "contains no .yaml or .yml files" in caplog.text

    def test_nonexistent_model_file_warns(self, tmp_path, caplog):
        """Individual model file that doesn't exist logs a warning."""
        import logging

        from mdb_engine.osi.loader import load_all_models

        missing_file = str(tmp_path / "nonexistent.yaml")
        config = {"enabled": True, "models": [missing_file]}
    ...
    def test_zero_models_catch_all_warning(self, tmp_path, caplog):
        """OSI enabled but zero models loaded logs a summary warning."""
        import logging

        from mdb_engine.osi.loader import load_all_models

        bogus_path = str(tmp_path / "missing")
        config = {"enabled": True, "models_path": bogus_path}

        with caplog.at_level(logging.WARNING):
            load_all_models(config)

        assert "no models were loaded" in caplog.text
        assert "Sources tried" in caplog.text


class TestLoaderFileProvenance:
    """Tests for _source_file tracking in loaded models."""

    def test_source_file_injected(self, tmp_path):
        """Each loaded model contains _source_file from the YAML file."""
        try:
            import yaml
        except ImportError:
            pytest.skip("PyYAML not installed")
        ...
        model_file = tmp_path / "movies.yaml"
        model_file.write_text(yaml.dump({"semantic_model": [{"name": "movies_model", "datasets": []}]}))

        models = load_osi_models(str(model_file))
        assert len(models) == 1
        assert models[0].get("_source_file") == str(model_file)

    def test_source_file_from_directory(self, tmp_path):
        """Models loaded from a directory have _source_file set to their file."""
        try:
            import yaml
        except ImportError:
            pytest.skip("PyYAML not installed")
        ...
        (tmp_path / "a.yaml").write_text(yaml.dump({"semantic_model": [{"name": "model_a", "datasets": []}]}))
        (tmp_path / "b.yaml").write_text(yaml.dump({"semantic_model": [{"name": "model_b", "datasets": []}]}))

        models = load_osi_models(str(tmp_path))
        assert len(models) == 2
        source_files = {m["_source_file"] for m in models}
        assert str(tmp_path / "a.yaml") in source_files
        assert str(tmp_path / "b.yaml") in source_files

    def test_validation_error_includes_source_file(self, tmp_path, caplog):
        """Validation error log includes the source file path."""
        import logging
        ...
        # Model missing required 'name' field -- triggers validation error
        bad_file = tmp_path / "bad.yaml"
        bad_file.write_text(yaml.dump({"semantic_model": [{"datasets": []}]}))
...
def _load_family_model() -> dict[str, Any]:
    """Load the family management model from the sso-app-3 YAML file."""
    from pathlib import Path

    yaml_path = Path(__file__).parent.parent.parent / (
        "examples/advanced/sso-multi-app/apps/sso-app-3/semantic_models/family_management.yaml"
    )
    if not yaml_path.exists():
        pytest.skip("sso-app-3 family_management.yaml not found")
...
class TestOsiModelStore:
    ...
    async def test_seed_first_run(self, mock_collection):
        ...
        config = {"models_path": "semantic_models/"}

        seeded = await store.seed_from_config(config)

        assert seeded is True
        # Should have called update_one for the meta doc at minimum
    ...
    @pytest.mark.asyncio
    async def test_seed_hash_mismatch_reseeds(self, mock_collection, tmp_path):
        ...
        # Create actual YAML files so the seeder finds models to load
        (tmp_path / "model.yaml").write_text(
            yaml.dump({"semantic_model": [{"name": "test_model", "datasets": [], "metrics": []}]})
        )
    ...
    def test_hash_deterministic(self):
        ...
        config = {"models_path": "semantic_models/", "models": ["a.yaml"]}
    ...
    def test_hash_changes_with_config(self):
        ...
        h1 = OsiModelStore._compute_config_hash({"models_path": "models_a/"})
        h2 = OsiModelStore._compute_config_hash({"models_path": "models_b/"})
        assert h1 != h2
...
class TestFamilyModelStructure:
    ...
    def test_model_has_17_datasets(self):
        """Family model has exactly 17 datasets."""
        model = _load_family_model()
    ...
    def test_model_has_22_relationships(self):
        """Family model has 22 relationships."""
        model = _load_family_model()
        relationships = model.get("relationships", [])
        assert len(relationships) == 22

    def test_model_has_11_metrics(self):
        """Family model has 11 governed metrics."""
        model = _load_family_model()
        ...
        expected_names = {
            ...
            "meals_planned",
            "grocery_items_pending",
            "overdue_homework",
            "chores_completed_this_week",
...
class TestFamilyRegistryLoading:
    ...
    async def test_registry_loads_all_datasets(self):
        """Registry indexes all 17 family datasets."""
        model = _load_family_model()
        registry = _make_registry_with_models([model], app_slug="ai-chat")
    ...
    async def test_registry_loads_all_metrics(self):
        """Registry indexes all 11 family metrics."""
        model = _load_family_model()
        registry = _make_registry_with_models([model], app_slug="ai-chat")
    ...
    async def test_registry_loads_all_relationships(self):
        """Registry indexes all 22 relationships."""
        model = _load_family_model()
        registry = _make_registry_with_models([model], app_slug="ai-chat")
...
class TestFamilySynonymResolution:
    ...
    def test_chore_resolves_to_chore(self):
        """'chore' resolves to the chore dataset (dedicated dataset)."""
        registry = _make_family_registry()
        ds = registry.get_dataset("chore")
        assert ds is not None
        assert ds["name"] == "chore"
...
class TestFamilyPromptEnrichment:
    ...
    def test_node_types_extracted_from_family_model(self):
        """extract_node_types_from_osi returns all 17 family dataset names."""
        from mdb_engine.graph.osi_loader import extract_node_types_from_osi
    ...
    def test_metric_names_extracted_from_family_model(self):
        """extract_metric_names returns all 11 family metrics with synonyms."""
        from mdb_engine.graph.osi_loader import extract_metric_names
...
class TestOsiValidator:
    """Tests for mdb_engine.osi.validator."""

    def test_valid_model_passes(self):
        """A well-formed model returns .valid=True."""
        from mdb_engine.osi.validator import validate_osi_model

        result = validate_osi_model(_sample_osi_model())
        assert result.valid
        assert len(result.errors) == 0
        assert result.models_checked == 1

    def test_missing_name_fails(self):
        """Model without name returns error."""
        from mdb_engine.osi.validator import validate_osi_model

        result = validate_osi_model({"datasets": []})
        assert not result.valid
        assert any("name" in e.message for e in result.errors)

    def test_empty_dict_fails(self):
        """Empty dict returns error for missing name."""
        from mdb_engine.osi.validator import validate_osi_model

        result = validate_osi_model({})
        assert not result.valid

    def test_non_dict_fails(self):
        """Passing a non-dict returns error."""
        from mdb_engine.osi.validator import validate_osi_model

        result = validate_osi_model("not a dict")  # type: ignore[arg-type]
        assert not result.valid
        assert any("dict" in e.message for e in result.errors)

    def test_dataset_missing_name_fails(self):
        """Dataset without name field errors."""
        from mdb_engine.osi.validator import validate_osi_model

        model = {"name": "test", "datasets": [{"fields": []}]}
        result = validate_osi_model(model)
        assert not result.valid
        assert any("name" in e.message for e in result.errors)

    def test_duplicate_dataset_names_warns(self):
        """Duplicate dataset names produce warning."""
        from mdb_engine.osi.validator import validate_osi_model

        model = {
            "name": "test",
            "datasets": [
                {"name": "customer", "ai_context": {"synonyms": ["client"]}},
                {"name": "customer", "ai_context": {"synonyms": ["buyer"]}},
            ],
        }
        result = validate_osi_model(model)
        assert any("Duplicate" in w.message for w in result.warnings)

    def test_dataset_without_synonyms_warns(self):
        """Dataset without ai_context.synonyms warns."""
        from mdb_engine.osi.validator import validate_osi_model

        model = {"name": "test", "datasets": [{"name": "widget"}]}
        result = validate_osi_model(model)
        assert any("synonyms" in w.message.lower() for w in result.warnings)

    def test_relationship_references_unknown_dataset_warns(self):
        """Relationship referencing non-existent dataset warns."""
        from mdb_engine.osi.validator import validate_osi_model

        model = {
            "name": "test",
            "datasets": [{"name": "actor", "ai_context": {"synonyms": ["star"]}}],
            "relationships": [
                {"name": "starred_in", "left_dataset": "actor", "right_dataset": "movie"},
            ],
        }
        result = validate_osi_model(model)
        assert any("movie" in w.message for w in result.warnings)

    def test_invalid_synonyms_type_fails(self):
        """Synonyms as a string (not list) errors."""
        from mdb_engine.osi.validator import validate_osi_model

        model = {
            "name": "test",
            "datasets": [{"name": "widget", "ai_context": {"synonyms": "not a list"}}],
        }
        result = validate_osi_model(model)
        assert not result.valid
        assert any("list" in e.message for e in result.errors)

    def test_validate_osi_yaml_valid(self):
        """Valid YAML string passes."""
        from mdb_engine.osi.validator import validate_osi_yaml
...
semantic_model:
  - name: test_model
    datasets:
      - name: widget
        ai_context:
          synonyms: [gadget, thing]
"""
        result = validate_osi_yaml(yaml_str)
        assert result.valid

    def test_validate_osi_yaml_invalid_syntax(self):
        """Broken YAML returns parse error."""
        from mdb_engine.osi.validator import validate_osi_yaml

        result = validate_osi_yaml("{ invalid: yaml: [[[")
        assert not result.valid
        assert any("parse" in e.message.lower() or "YAML" in e.message for e in result.errors)

    def test_validate_osi_yaml_empty(self):
        """Empty string returns error."""
        from mdb_engine.osi.validator import validate_osi_yaml

        result = validate_osi_yaml("")
        assert not result.valid

    def test_validation_result_to_dict(self):
        """OsiValidationResult.to_dict() returns structured output."""
        from mdb_engine.osi.validator import validate_osi_model
...
class TestOsiScaffold:
    """Tests for mdb_engine.osi.scaffold."""

    def test_scaffold_creates_datasets_from_node_types(self):
        """Each node_type becomes a dataset."""
        from mdb_engine.osi.scaffold import scaffold_osi_model

        model = scaffold_osi_model("test", node_types=["actor", "movie", "genre"])
        ds_names = {ds["name"] for ds in model["datasets"]}
        assert "actor" in ds_names
        assert "movie" in ds_names
        assert "genre" in ds_names

    def test_scaffold_generates_synonyms_for_known_types(self):
        """Known types like 'actor' get rich synonyms."""
        from mdb_engine.osi.scaffold import scaffold_osi_model

        model = scaffold_osi_model("test", node_types=["actor"])
        actor_ds = next(ds for ds in model["datasets"] if ds["name"] == "actor")
        synonyms = actor_ds["ai_context"]["synonyms"]
        assert "actor" in synonyms
        assert "star" in synonyms
        assert len(synonyms) >= 5

    def test_scaffold_generates_fallback_synonyms(self):
        """Unknown types get basic synonyms."""
        from mdb_engine.osi.scaffold import scaffold_osi_model

        model = scaffold_osi_model("test", node_types=["custom_widget"])
        ds = next(ds for ds in model["datasets"] if ds["name"] == "custom_widget")
        synonyms = ds["ai_context"]["synonyms"]
        assert "custom_widget" in synonyms
        assert "custom widget" in synonyms

    def test_scaffold_adds_user_preference_with_categories(self):
        """Categories create user_preference dataset."""
        from mdb_engine.osi.scaffold import scaffold_osi_model

        model = scaffold_osi_model("test", node_types=["movie"], categories=["favorites", "watchlist"])
        ds_names = {ds["name"] for ds in model["datasets"]}
        assert "user_preference" in ds_names
        pref_ds = next(ds for ds in model["datasets"] if ds["name"] == "user_preference")
        synonyms = pref_ds["ai_context"]["synonyms"]
        assert "favorites" in synonyms
        assert "watchlist" in synonyms

    def test_scaffold_generates_relationships(self):
        """actor+movie creates starred_in relationship."""
        from mdb_engine.osi.scaffold import scaffold_osi_model

        model = scaffold_osi_model("test", node_types=["actor", "movie", "director", "genre"])
        rel_names = {r["name"] for r in model["relationships"]}
        assert "starred_in" in rel_names
        assert "directed" in rel_names
        assert "genre_of" in rel_names

    def test_scaffold_model_validates(self):
        """Scaffolded model passes the validator."""
        from mdb_engine.osi.scaffold import scaffold_osi_model
        from mdb_engine.osi.validator import validate_osi_model
    ...
    def test_scaffold_to_yaml_produces_valid_yaml(self):
        """YAML output parses cleanly."""
        import yaml as yaml_lib

        from mdb_engine.osi.scaffold import scaffold_to_yaml

        yaml_str = scaffold_to_yaml("test", node_types=["actor", "movie"])
        data = yaml_lib.safe_load(yaml_str)
        assert data is not None
        assert "semantic_model" in data
        assert len(data["semantic_model"]) == 1

    def test_scaffold_to_yaml_has_header_comment(self):
        """YAML output starts with header comment."""
        from mdb_engine.osi.scaffold import scaffold_to_yaml

        yaml_str = scaffold_to_yaml("test", node_types=["actor"])
        assert yaml_str.startswith("# ---")
        assert "Auto-Scaffolded" in yaml_str
...
class TestContentHash:
    """Tests for content-based YAML hashing in OsiModelStore."""

    def test_hash_changes_when_yaml_content_changes(self, tmp_path):
        """Modifying YAML content changes the hash."""
        from mdb_engine.osi.store import OsiModelStore
    ...
    def test_hash_ignores_non_yaml_files(self, tmp_path):
        """Non-YAML files don't affect the hash."""
        from mdb_engine.osi.store import OsiModelStore
        ...
        # Add a .txt file -- should not change hash
        (tmp_path / "notes.txt").write_text("this is not yaml")
        h2 = OsiModelStore._compute_config_hash(config)
...
class TestWatcherReseed:
    """Tests for OsiModelWatcher file-change detection."""

    def test_first_scan_returns_false(self, tmp_path):
        """Initial scan sets baseline, returns False."""
        from mdb_engine.osi.registry import OsiModelRegistry
        from mdb_engine.osi.watcher import OsiModelWatcher

        (tmp_path / "model.yaml").write_text("name: test\n")

        config = {"enabled": True, "models_path": str(tmp_path)}
        registry = OsiModelRegistry(app_slug="test", config=config)

        watcher = OsiModelWatcher(registry=registry, interval_minutes=1)
        assert watcher.check_for_updates() is False

    def test_no_change_returns_false(self, tmp_path):
        """Unchanged files return False on second check."""
        from mdb_engine.osi.registry import OsiModelRegistry
        from mdb_engine.osi.watcher import OsiModelWatcher
        ...
        watcher = OsiModelWatcher(registry=registry, interval_minutes=1)
        watcher.check_for_updates()  # baseline
        assert watcher.check_for_updates() is False

    def test_check_for_updates_detects_file_change(self, tmp_path):
        """Modified file triggers True on next check."""
        import os
        import time
        ...
        yaml_file.write_text("name: v2_changed\n")
        future_time = time.time() + 10
        os.utime(yaml_file, (future_time, future_time))

        assert watcher.check_for_updates() is True
