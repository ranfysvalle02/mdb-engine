...
@pytest.mark.asyncio
class TestAppSecretErrors:
    """Tests for error handling paths in AppSecretsManager."""

    async def test_verify_base64_decode_error(self, app_secrets_manager, mock_mongo_db):
        """Base64 decode errors in verify_app_secret return False."""
        _, mock_collection = mock_mongo_db
        mock_collection.find_one.return_value = {
            "_id": "test_app",
            "encrypted_secret": "!!!not-base64!!!",
            "encrypted_dek": "!!!not-base64!!!",
        }
        result = await app_secrets_manager.verify_app_secret("test_app", "secret")
        assert result is False

    async def test_get_app_secret_decryption_failure(self, app_secrets_manager, mock_mongo_db, encryption_service):
        """Decryption failure in get_app_secret returns None."""
        _, mock_collection = mock_mongo_db
        mock_collection.find_one.return_value = {
            "_id": "test_app",
            "encrypted_secret": base64.b64encode(b"garbage-ciphertext").decode(),
            "encrypted_dek": base64.b64encode(b"garbage-dek").decode(),
        }
        result = await app_secrets_manager.get_app_secret("test_app")
        assert result is None

    async def test_store_app_secret_pymongo_error(self, app_secrets_manager, mock_mongo_db):
        """PyMongoError during store_app_secret raises OperationFailure."""
        from pymongo.errors import OperationFailure, PyMongoError

        _, mock_collection = mock_mongo_db
        mock_collection.find_one = AsyncMock(side_effect=PyMongoError("connection lost"))

        with pytest.raises(OperationFailure, match="Failed to store app secret"):
            await app_secrets_manager.store_app_secret("test_app", "my_secret")
