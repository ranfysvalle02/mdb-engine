...
class ParallaxEngine:
    ...
    def __init__(self, openai_client, db, watchlist: Optional[List[str]] = None, deployment_name: str = "gpt-4o", github_token: Optional[str] = None):
        """
        ...
        if not self.github_token:
            raise RuntimeError("GITHUB_TOKEN is required for GitHub GraphQL API. Set it as an environment variable or pass it to ParallaxEngine.")
    ...
    def _search_repos_via_graphql(self, keyword: str, min_stars: int = 50, language: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for repo in repos:
                ...
                if files_found:
                    ...
                    if keyword_found:
                        logger.info(f"   ✅ Found: {name_with_owner} (⭐ {stars}) - Files: {file_names} [keyword '{keyword}' in files]")
                    elif keyword_in_desc:
                        logger.info(f"   📋 Found: {name_with_owner} (⭐ {stars}) - Files: {file_names} [keyword '{keyword}' in description, analyzing files]")
                    else:
                        logger.info(f"   📋 Found: {name_with_owner} (⭐ {stars}) - Files: {file_names} [will analyze for relevance]")
                    ...
                    valid_findings.append({
                        ...
                        "files_found": [f["filename"] for f in files_found],  # List of all files found
                        "file_found": files_found[0]["filename"],  # Primary file (for backward compatibility)
                        "full_content": combined_content,  # Combined content from all files
                        "files": {f["filename"]: f["content"] for f in files_found},  # Individual file contents
                    ...
                    logger.debug(f"   ⚠️  Skipping {name_with_owner}: No README.md, AGENTS.md, or LLMs.md found")
    ...
    async def _search_and_filter_repos(self) -> List[Dict[str, Any]]:
        ...
        tasks = [
            loop.run_in_executor(None, self._search_repos_via_graphql, keyword, self.min_stars, self.language_filter)
        ...
        logger.info(f"Found {len(all_findings)} unique repositories with AGENTS.md or LLMs.md files")
        return all_findings
    ...
    async def _generate_viewpoint(
        ...
        try:
            ...
            format_instructions_str = f"Respond with valid JSON matching this schema: {format_instructions}"
            ...
            logger.debug(f"{lens_name} Viewpoint generated successfully for {repo_owner}/{repo_name}")
            return result
    ...
    async def analyze_repositories(self, progress_callback=None) -> List[ParallaxReport]:
        ...
        logger.info(f"Starting analysis: searching GitHub for keywords: {self.watchlist} (min_stars: {self.min_stars})")
        ...
        for repo in repos:
            ...
            file_found = repo.get('file_found', files_found[0] if files_found else 'README.md')  # Primary file (for backward compatibility)
            ...
            logger.info(f"Orchestrating Parallax View for: {repo_id} (⭐ {stars}) - Analyzing files: {files_str}")
            ...
            task_relevance = self._generate_viewpoint(
                repo_name, repo_owner, url, file_content, files_str,  # Pass files list as string
                "Relevance", watchlist=self.watchlist
            )
            task_technical = self._generate_viewpoint(
                repo_name, repo_owner, url, file_content, files_str,
                "Technical"
            )
            ...
            if relevance_dict or technical_dict:
                ...
                logger.info(f"Parallax report stored for: {repo_id} (relevance: {bool(relevance_dict)}, technical: {bool(technical_dict)})")
        ...
        logger.info(f"Analysis complete: {new_repos} new repos analyzed, {cached_repos} cached, {len(reports)} reports generated")
