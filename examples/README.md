...
- `MDB_MONGO_URI` - MongoDB connection string (uses Docker service name)
...
# Initialize engine (reads MDB_MONGO_URI from env, or defaults to localhost)
engine = MongoDBEngine()
await engine.initialize()
...
engine = MongoDBEngine()  # reads MDB_MONGO_URI from env, defaults to localhost
