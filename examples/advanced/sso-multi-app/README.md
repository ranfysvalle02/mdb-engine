...
graph TB
    ...
    Member[Member<br/>Cognitive Memory Showcase<br/>- Memory Types & Timelines<br/>- GraphRAG & Profiles<br/>- OSI Semantic Models]
    
    MongoDB[(MongoDB<br/>Shared Database)]
    SampleMflix[(sample_mflix<br/>Movie Dataset)]
    ...
    AuthHub -->|Issues JWT Tokens| Member
    
    SSOApp1 -->|Validates Tokens| UserPool
    SSOApp2 -->|Validates Tokens| UserPool
    Member -->|Validates Tokens| UserPool
    
    AuthHub --> MongoDB
    SSOApp1 --> MongoDB
    SSOApp2 --> MongoDB
    Member --> MongoDB
    Member -->|Co-star Network| SampleMflix
```
...
## Sample Dataset (sample_mflix)

The **Member** app uses the official MongoDB `sample_mflix` dataset for its co-star network and `$graphLookup` demos. This is a one-time setup step (~350 MB download).

> **Note:** The Member app works without `sample_mflix` -- the "Seed Demo Data" button creates its own sample movies. The full dataset just makes the co-star network richer.

**Setup (one-time, then automatic):**

```bash
# 1. Download the archive (~350 MB) -- only need to do this once
./download-sample-dataset.sh

# 2. Start everything -- sample_mflix is restored automatically on first boot
docker-compose up --build
```

`docker-compose up` will fail with a clear error if the archive hasn't been downloaded yet. On subsequent runs the restore is skipped (idempotent).

**Requires:** `curl` (for the download script).
...
- **Functionality**: Comprehensive demo of MDB-Engine's memory & knowledge features
- **Required Role**: `base_user`
- **Uses**: `sample_mflix` dataset (auto-seeded by Docker Compose)
- **Features demonstrated**:
  - Ebbinghaus Forgetting Curve (server-side decay with time machine)
  - Flashbulb Memory (emotion-based stability boosts)
  - Memory Types (episodic, procedural, working, semantic)
  - Memory Timelines (fork / switch alternate realities)
  - Prospective Memory (intention-based triggers)
  - Shared Memory (privacy-safe group promotion)
  - Reflection & Consolidation lifecycle
  - Profile Service (auto-materialized from memories + graph)
  - GraphRAG with `$graphLookup` traversal and BFS path finding
  - OSI Semantic Models (entity resolution, metric routing, export)
  - Cold Storage (graceful forgetting with restore)
  - Conflict Detection (contradictory fact resolution)
  - Co-star Network (sample_mflix `$graphLookup` demo)
  - Persona Management (customizable AI personality)
...
- `MDB_MONGO_URI`: MongoDB connection string
- `MDB_DB_NAME`: Database name (must be same for all apps)
- `MDB_JWT_SECRET`: JWT secret (must be same for all apps for SSO to work)
...
export MDB_JWT_SECRET="your-generated-secret-here"

# Or create a .env file (DO NOT commit to git!)
echo "MDB_JWT_SECRET=your-generated-secret-here" > .env
```
...
1. **Shared JWT Secret**: All apps must use the same `MDB_JWT_SECRET`
...
export MDB_DB_NAME="oblivio_apps"
export MDB_JWT_SECRET="your-secret-key"
```
...
- **Check JWT secret**: All apps must use the same `MDB_JWT_SECRET`
- **Check database**: All apps must use the same `MDB_DB_NAME`
...
- [ ] **JWT Secret**: Set `MDB_JWT_SECRET` to a strong, randomly generated secret (32+ bytes)
...
3. **Set environment variables:**
   - `MDB_MONGO_URI` - Your MongoDB connection string
   - `MDB_DB_NAME` - Database name
   - `MDB_JWT_SECRET` - JWT secret (same for all apps)
...
├── download-sample-dataset.sh   # One-time sample_mflix data loader
├── start-all-apps.py            # Bundled startup script
├── multi_app_manifest.json      # Multi-app manifest
├── .env.example                 # Environment template
└── apps/
    ├── multi_app_main.py         # Multi-app main file
    ├── # shared_security.py removed — use mdb_engine.auth imports directly
    ...
    │   ├── web.py
    │   └── templates/
    ├── member/                  # Member (Cognitive Memory Showcase)
    │   ├── manifest.json        # profile_config, osi, memory_types, etc.
    │   ├── web.py               # 36+ API routes across 7 feature areas
    │   ├── semantic_models/     # OSI YAML (movie knowledge model)
    │   └── templates/
