...
help:
	...
	@echo "  make update-badges    - Update README.md badges from coverage.json (CI use)"
...
update-badges:
	...
	@if [ ! -f coverage.json ]; then \
		echo "No coverage.json found. Run 'make test-coverage' first."; \
		exit 1; \
	fi
	@$(PYTHON) scripts/update_coverage_badges.py
